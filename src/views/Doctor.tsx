...
export function Doctor() {
  ...
  // Auto-save SOAP and Diagnosis with debounce
  React.useEffect(() => {
  ...
  }, [soap, diagnosis, selectedVisitId, updateSOAP]);
  ...
  return (
    <>
      <div className="grid grid-cols-12 gap-6 p-4 lg:p-6 pb-24">
      {/* Scroll Controls Floating */}
      <div className="fixed bottom-12 right-12 z-[90] flex flex-col gap-3 group pointer-events-none">
        <button 
          onClick={scrollToTop}
          className="pointer-events-auto p-4 bg-brand-blue/10 backdrop-blur-md border border-brand-blue/20 text-brand-blue rounded-full shadow-2xl hover:bg-brand-blue hover:text-white transition-all transform hover:scale-110 active:scale-95"
          title="Scroll to Top"
        >
      ...
      <section className="col-span-12 lg:col-span-3 flex flex-col gap-6 shrink-0">
        {/* Doctor's Queue */}
        <div className="bento-card p-0 flex flex-col h-[480px] overflow-hidden shadow-xl">
        ...
        <div className="bento-card-dark p-0 border-slate-800 flex flex-col h-[320px] overflow-hidden shadow-2xl">
              ...
              <div className="col-span-12 lg:col-span-8 flex flex-col gap-4 h-full">
                <div className="bento-card p-8 flex-1 flex flex-col">
                  ...
                  <div className="flex-1 overflow-y-auto custom-scrollbar pr-2">
                                   ...
                                   >
                                     ...
                                     {(isExpanded) && (
                                       <motion.div 
                                         ...
                                         className="pt-3 border-t border-emerald-100/50 flex justify-center"
                                       >
                                         <button 
                                           onClick={() => setViewingReportOrder(order)}
                                           className="px-6 py-2.5 bg-emerald-500 text-white rounded-xl text-[10px] font-black uppercase tracking-[0.2em] shadow-lg shadow-emerald-500/20 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center gap-2"
                                         >
                                           <FileText className="w-3.5 h-3.5" />
                                           View Full Diagnostic Report // عرض التقرير الكامل
                                         </button>
                                       </motion.div>
                                     )}
                    ...
                    {activeTab === 'history' && (
                      <div className="space-y-6 pb-8 h-full overflow-y-auto max-h-[550px] custom-scrollbar">
                        {patientHistory.slice().reverse().map(prevVisit => (
                          <div key={prevVisit.id} className="p-8 bg-slate-50 border border-slate-100 rounded-[32px] relative overflow-hidden group">
                            <div className="flex justify-between items-start mb-6">
                              <div>
                                <span className="px-2 py-1 bg-brand-blue/5 text-brand-blue text-[9px] font-black rounded uppercase tracking-widest">
                                  Consultation Matrix: {format(new Date(prevVisit.createdAt), 'MMM d, yyyy')}
                                </span>
                                <h3 className="text-sm font-black text-slate-800 uppercase italic mt-1">{prevVisit.diagnosis || 'No Diagnosis Recorded'}</h3>
                              </div>
                              <span className="text-[8px] font-mono font-bold text-slate-300">REF_{prevVisit.id.slice(-6)}</span>
                            </div>

                            <div className="grid grid-cols-2 gap-4 mb-6">
                               <div className="p-4 bg-white/50 rounded-2xl border border-white">
                                  <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1 italic">Subjective</p>
                                  <p className="text-[11px] font-bold text-slate-600 leading-relaxed">{prevVisit.soapNotes?.subjective || '---'}</p>
                               </div>
                               <div className="p-4 bg-white/50 rounded-2xl border border-white">
                                  <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1 italic">Objective</p>
                                  <p className="text-[11px] font-bold text-slate-600 leading-relaxed">{prevVisit.soapNotes?.objective || '---'}</p>
                               </div>
                               <div className="p-4 bg-white/50 rounded-2xl border border-white">
                                  <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1 italic">Assessment</p>
                                  <p className="text-[11px] font-bold text-slate-600 leading-relaxed">{prevVisit.soapNotes?.assessment || '---'}</p>
                               </div>
                               <div className="p-4 bg-white/50 rounded-2xl border border-white">
                                  <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1 italic">Plan</p>
                                  <p className="text-[11px] font-bold text-slate-600 leading-relaxed">{prevVisit.soapNotes?.plan || '---'}</p>
                               </div>
                            </div>

                            {/* Diagnostic Results from this specific past visit */}
                            {prevVisit.orders && prevVisit.orders.some(o => o.status === 'Completed') && (
                              <div className="space-y-3">
                                 <p className="text-[9px] font-black text-brand-blue uppercase tracking-widest mb-2 italic flex items-center gap-2">
                                   <Beaker className="w-3 h-3" /> Historical Diagnostic Stream
                                 </p>
                                 <div className="grid grid-cols-2 gap-3">
                                    {prevVisit.orders.filter(o => o.status === 'Completed').map(order => (
                                      <button 
                                        key={order.id}
                                        onClick={() => setViewingReportOrder(order)}
                                        className="p-3 bg-white border border-slate-100 rounded-xl text-left flex justify-between items-center group/btn hover:border-emerald-500/30 transition-all"
                                      >
                                        <div className="overflow-hidden">
                                          <p className="text-[10px] font-black text-slate-700 uppercase italic truncate">{order.description}</p>
                                          <p className="text-[8px] font-bold text-slate-400 uppercase">{order.type}</p>
                                        </div>
                                        <FileText className="w-4 h-4 text-emerald-500 group-hover/btn:scale-110 transition-transform shrink-0" />
                                      </button>
                                    ))}
                                 </div>
                              </div>
